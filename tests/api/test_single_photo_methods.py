...
class TestGetSinglePhoto:
    ...
    def test_sets_realname_to_none_if_empty(self, api: FlickrApi) -> None:
        """
        If the photo's owner doesn't have a realname, then
        ``owner.realname`` is None.
        """
    ...
    def test_sets_path_alias_to_none_if_empty(self, api: FlickrApi) -> None:
        """
        If the photos' owner doesn't have a path alias, then
        ``owner.path_alias`` is None.
        """
    ...
    def test_sets_granularity_on_date_taken(self, api: FlickrApi) -> None:
        """
        Get the granularity of the date taken.

        In particular, this should be a human-readable string and not
        one of the numeric codes returned by Flickr.
        """
    ...
    def test_sets_date_unknown_on_date_taken(
        ...
        """
        If a photo doesn't have a date taken, or the date taken value
        is obviously nonsensical, then ``date_taken`` is None.
        """
    ...
    def test_empty_photo_description_is_none(self, api: FlickrApi) -> None:
        """
        If a photo has an empty/missing description, then ``description``
        is None.
        """
    ...
    def test_empty_photo_title_is_none(self, api: FlickrApi) -> None:
        """
        If a photo has an empty/missing title, then ``title`` is None.
        """
    ...
    def test_get_empty_tags_for_untagged_photo(self, api: FlickrApi) -> None:
        """
        If a photo is untagged, then the list of tags is empty.
        """
    ...
    def test_get_empty_location_for_photo_without_geo(self, api: FlickrApi) -> None:
        """
        Get a photo that doesn't have any location data.
        """
    ...
    def test_it_discards_location_if_accuracy_is_zero(self, api: FlickrApi) -> None:
        """
        Discard location information if the accuracy is so high as to
        be unusable.
        """
    ...
    def test_it_can_get_a_video(self, api: FlickrApi) -> None:
        """
        Get a "photo" which is really a video.
        """
    ...
    def test_a_photo_with_an_empty_tag(self, api: FlickrApi) -> None:
        """
        Get a photo with an empty string as a tag.
        """
    ...
    def test_fixes_realname(self, api: FlickrApi, photo_id: str, realname: str) -> None:
        """
        We apply the realname "fixes" we have hard-coded for photos
        uploaded by users of particular interest.
        """
        photo = api.get_single_photo(photo_id=photo_id)
    ...
    def test_gets_raw_tag_information(self, api: FlickrApi) -> None:
        """
        Get the raw tag information for photos.
        """
...
def test_is_photo_deleted(api: FlickrApi, photo_id: str, is_deleted: bool) -> None:
    """
    Check if a photo has been deleted.
    """
...
class TestGetPhotoContexts:
    ...
    def test_gets_empty_lists_if_no_contexts(self, api: FlickrApi) -> None:
        """
        Get a photo that isn't in any albums/galleries/groups.
        """
    ...
    def test_gets_album_info(self, api: FlickrApi) -> None:
        """
        Get a photo which has been added to several albums.
        """
    ...
    def test_gets_gallery_info(self, api: FlickrApi) -> None:
        """
        Get a photo which has been added to several galleries.
        """
    ...
    def test_gets_group_info(self, api: FlickrApi) -> None:
        """
        Get a photo which has been added to several group pools.
        """
...
class TestPrivatePhotos:
    """
    Tests for ``SinglePhotoMethods`` with a private photo.

    It's difficult to find links to private photos to test with;
    ...
    def test_get_private_photo_is_error(self, api: FlickrApi) -> None:
        """
        Looking up a private photo throws ``PhotoIsPrivate``.
        """
    ...
    def test_get_private_photo_contexts_is_error(self, api: FlickrApi) -> None:
        """
        Looking up the context of a private photo throws ``ResourceNotFound``.
        """
        # TODO: This should really throw ``PhotoIsPrivate``.
